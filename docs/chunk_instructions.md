...
cargo test --package nexusvm --lib -- chunk::api::test::full_e2e_execution --exact --nocapture

The following show tests for step wise execution.

1. Generate Partial Scripts

RUST_BACKTRACE=full cargo test --package nexusvm --lib -- chunk::api::test::test_fn_compile --exact --nocapture --ignored

2. Generate Signed Assertions

RUST_BACKTRACE=full cargo test --package nexusvm --lib -- chunk::api::test::test_fn_generate_signatures --exact --nocapture --ignored

3. Validate generated Assertions

RUST_BACKTRACE=full cargo test --package nexusvm --lib -- chunk::api::test::test_fn_validate_assertions --exact --nocapture --ignored

4. Manually Corrupt each Assertion one at a time and Disprove invalid assertion

RUST_BACKTRACE=full cargo test --package nexusvm --lib -- chunk::api::test::test_fn_disprove_invalid_assertions --exact --nocapture --ignored
...
`api_compiletime_utils` - interfacing methods used during nexusvm compile time i.e. presigning

`api_runtime_utils` - interfacing methods used during nexusvm runtime i.e. assertion and disprove
...
20-byte hashes for 160-bit second preimage resistance security. We aren't concerned with collision resistance in nexusvm setup.
