...
fn main() {
    ...
    let network = env::var("TONDI_NETWORK").unwrap_or_else(|_| {
        ...
        "mainnet".to_string()
            ...
            println!("cargo:warning=Building for Bitcoin network: {}", network); // NOTE: Bitcoin network reference
...
fn get_guest_options(network: String) -> HashMap<&'static str, risc0_build::GuestOptions> {
    ...
    let opts = if env::var("REPR_GUEST_BUILD").is_ok() {
        ...
        let docker_opts = DockerOptionsBuilder::default()
            .root_dir(root_dir)
            .env(vec![("BITCOIN_NETWORK".to_string(), network.clone())]) // NOTE: Bitcoin network environment variable
            .build()
            .unwrap();
...
fn copy_binary_to_elfs_folder(network: String) {
    ...
    // base_dir will be /home/ozan/workspace/NexusVM/
    ...
    let logical_elf_path_for_id = match network.as_str() {
        ...
        "simnet" => "prover/elfs/simnet-header-chain-guest.bin",
        "devnet" => "prover/elfs/devnet-header-chain-guest.bin",
        _ => {
            println!(
