...
use nexusvm::chunk::api::type_conversion_utils::RawProof;
use nexusvm::chunk::api::{NUM_HASH, NUM_PUBS, NUM_U256};
use nexusvm::signatures::HASH_LEN;
...
pub async fn verify_funding_inputs(client: &NexusVMClient, funding_inputs: &Vec<(&Address, Amount)>) {
...
pub fn find_peg_in_graph(client: &NexusVMClient, peg_in_graph_id: &str) -> Option<PegInGraph> {
...
pub fn find_peg_out_graph(client: &NexusVMClient, peg_out_graph_id: &str) -> Option<PegOutGraph> {
...
pub fn get_intermediate_variables_cached() -> BTreeMap<String, usize> {
    ...
    intermediate_variables.unwrap_or_else(|| {
        ...
        for i in 0..NUM_HASH {
            intermediate_variables.insert(format!("{}", i + NUM_PUBS + NUM_U256), HASH_LEN);
        }
